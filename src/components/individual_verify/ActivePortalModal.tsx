import closeSVG from "../../assets/images/bclose.svg";
import Button from "../button/Button";

interface ActivePortalProps {
  active?: ()=>void
}

export default function ActivePortalModal(props: ActivePortalProps) {

  return (
    <>
    <div className="w-full absolute  bottom-[0px] block rounded-lg bg-white border-white shadow-[0_0px_10px_0px_rgba(0,0,0,0.3)] border z-10">

      <div className="px-8 py-5">
        <img className="float-end" src={closeSVG} alt="close" />
        <p className="font-semibold text-[20px] text-center pt-6 leading-6">Activate Portal 1</p>
        <p className="font-normal text-base text-center pt-4">
          You can have a maximum of 3 active Verification Portals. After activating your Portal, you can add people to your portal to <b>request their Human Likeness Verification</b> and keep track of their verification status.
        </p>
        <div className="mt-5">
          <label className="font-normal text-base leading-[22px]">Name your Portal 1</label>
          <input placeholder="Maximum 12 characters" className="w-full h-9 border border-[#D3D3D3] rounded-md p-2 placeholder:text-[16px] leading-[19px] font-normal hover:outline-black outline-1 mt-1"/>
        </div>
        <div className="py-2">
          <Button background={true} text="Activate" onClick={props.active}/>
          <Button background={false} text="Cancel"/>
        </div> 
      </div>
    </div>
    </>
  )
}
